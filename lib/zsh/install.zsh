...
ziextract() {
  ...
  case "${${ext:+.$ext}:-$file}" in
    ...
    ((#i)*.dmg)
      →zi-extract() {
        ...
        if (( retval )) {
          +zi-message "{annex}ziextract{ehi}:{warn} Problem occurred when attempted to copy the files from the mounted image{ehi}:{rst} \`{file}${file}{rst}'"
        }
        return $retval
      }
      ;;
  ...
  if [[ $(typeset -f + →zi-extract) == "→zi-extract" ]] {
    .zi-extract-wrapper "$file" →zi-extract || {
      +zi-message -n "{annex}ziextract{ehi}:{warn} Problem occurred while extracting \`{file}${file}{warn}'{rst}"
      local -a bfiles
      bfiles=( ._backup/*(DN) )
      if (( ${#bfiles} && !nobkp )) {
  ...
  if [[ ${#execs} -gt 0 ]] {
    ...
    if (( !OPTS[opt_-q,--quiet] )) {
      if (( ${#execs} == 1 )); then
        +zi-message "{annex}ziextract{ehi}: {auto}Successfully extracted and assigned \`+x\` chmod to the file '${execs[1]}'"
      else
        local sep="$ZI[col-rst],$ZI[col-obj] "
        if (( ${#execs} > 7 )) {
           +zi-message "{annex}ziextract{ehi}: {auto}Successfully extracted and assigned \`+x\` chmod to the files ({p}${(pj:$sep:)${(@)execs[1,5]:t}}{rst}) contained" \
            "{auto}in '$file'. All the extracted '${#execs}' executables are available in the \"INSTALLED_EXECS\" array"
        } else {
          +zi-message "{annex}ziextract{ehi}: {auto}Successfully extracted and assigned \`+x\` chmod to the files ({p}${(pj:$sep:)${execs[@]:t}}{rst}) {auto}contained in '$file'"
        }
      fi
    }
  } elif (( warning )) {
    +zi-message "{annex}ziextract{ehi}: {auto}Did not recognize the archive type of '${file}'" \
    "${ext:+/ ${ext} } ({error}no extraction has been done{rst})"
  }
  if (( move | move2 )) {
    local -a files
...
.zi-extract() {
  ...
  (
    builtin cd -q "$local_dir" || {
      +zi-message "{error}ERROR{ehi}: {auto}The path of the $tpe ({p}$local_dir{rst}) {auto}isn't accessible"
      return 1
    }
    local -a files
    files=( ${(@)${(@s: :)${extract##(\!-|-\!|\!|-)}}//(#b)(((#s)|([^\\])[\\]([\\][\\])#)|((#s)|([^\\])([\\][\\])#)) /${match[2]:+$match[3]$match[4] }${match[5]:+$match[6]${(l:${#match[7]}/2::\\:):-} }} )
    if [[ ${#files} -eq 0 && -n ${extract##(\!-|-\!|\!|-)} ]] {
        +zi-message "{error}ERROR{ehi}: {auto}The files ({p}${extract##(\!-|-\!|\!|-)}{rst}) not found, cannot extract"
        return 1
    } else {
      (( !${#files} )) && files=( "" )
    }
