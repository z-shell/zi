...
.zi-load () {
  ...
  if [[ $___user != % && ! -d ${ZI[PLUGINS_DIR]}/${___id_as//\//---} ]] {
    ...
    # own load has ZSRV_ID set and must still source the plugin.
    (( ${+ICE[pack]} && ${+ICE[service]} )) && [[ -z $ZSRV_ID ]] && ___pack_service=1
  }
  ZI_SICE[$___id_as]=
      ...
      while (( $# )) {
        ...
        if [[ -n $1 ]] {
          ...
          if (( ___action_load || !ZI[HAVE_SCHEDULER] )) {
            ...
            (( !___turbo && ${+ICE[pack]} && ${+ICE[service]} )) && [[ -z $ZSRV_ID ]] && ___turbo=1
          }
