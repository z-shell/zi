<center>**Version: 1.1,  11/22/2021**</center>

# Zsh Plugin Standard

This document defines the Zsh Plugin Standard. ZI fully supports
this standard.

This document is also available as a
[**PDF**](http://z-shell.github.io/ZSH-TOP-100/Zsh-Plugin-Standard.pdf).
...
1. Plugin managers: ZI, Zgen (after and if the
...
1. One plugin manager, ZI, implements plugin unloading and calls the
   function.
...
It's a recent addition to the standard and only one plugin manager, ZI,
implements it.
...
ZI uses this name and places this directory at `~/.zi/polaris` by
default.
...
One plugin manager, ZI, provides the `$ZPFX` parameter.
...
2. `:`: for hook-like functions, so it should be used e.g.: for the
   ...
   other
   custom hook-like mechanism in the plugin (e.g.: ZI annexes
   [**use**](http://z-shell.github.io/zi/wiki/Annexes/#how_to_code_them) such
   prefix for the ZI hook functions). Example function name:
   `:prompt_zinc_precmd`.
