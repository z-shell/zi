...
`zi` commands in `.zshrc`. It allows to perform the following actions on
...
2. Making changes (like removal of a plugin, i.e. of `zi load {removed-plugin}` command together with the possible associated `zi ice …` command) and then…
