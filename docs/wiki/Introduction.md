...
The command `zi ice` provides ice-modifiers for single next command (see
the README subsection
[**ice-modifiers**](https://github.com/z-shell/zi#ice-modifiers)). The
logic is that "ice" is something something that’s added (e.g. to a drink or a
coffee) – and in the ZI sense this means that ice is a modifier added to
the next ZI command, and also something that melts (so it doesn’t last
long) – and in the ZI use it means that the modifier lasts for only single
next ZI command. Using one other ice-modifier "**pick**" user can
...
commits are to be fetched**. So in summary, when user runs `zi update b4b4r07/httpstat` to update this plugin, and there are new commits, what happens
...
completions. ZI uses own method (based on symlinks instead of adding a
...
The exclamation mark causes ZI to reset the prompt after loading plugin – it
...
### A Quick Glance At The [For-Syntax](https://z-shell-github.io/zi/wiki/For-Syntax/)
...
files](https://z-shell.github.io/zi/wiki/Sourcing-multiple-files/). For more
information refer to the page dedicated to the new syntax
([here](https://z-shell.github.io/zi/wiki/For-Syntax/)).
...
[add-zsh-hook](http://z-shell.github.io/zi/wiki/zsh-plugin-standard/#use_of_add-zsh-hook_to_install_hooks)
...
tracking (so `zi load …`, not `zi light …`). Tracking causes
