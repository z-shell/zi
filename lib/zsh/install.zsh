...
.zi-get-package() {
  ...
  if [[ $profile != ./* ]]; then
    if ! .zi-download-file-stdout $URL 0 1 2>/dev/null > $tmpfile; then
      ...
      .zi-download-file-stdout $URL 1 1 2>/dev/null >1 $tmpfile
    fi
  else
    tmpfile=${profile%:*}
  ...
  for required ( $req ) {
    if [[ $required == (bgn|dl|rdl) ]]; then
      if [[ ( $required == bgn && \
      -z ${(k)ZI_EXTS[(r)<-> z-annex-data: z-a-bin-gem-node *]} ) || \
      ( $required == dl && -z ${(k)ZI_EXTS[(r)<-> z-annex-data: z-a-patch-dl *]} ) || \
      ( $required == rdl && -z ${(k)ZI_EXTS[(r)<-> z-annex-data: z-a-readurl *]} ) ]]; then
        local -A namemap
    ...
    (
      ...
      if ! .zi-download-file-stdout "$URL" 0 1 >! "$fname"; then
        if ! .zi-download-file-stdout "$URL" 1 1 >! "$fname"; then
          command rm -f "$fname"
          +zi-message "Download of the file {apo}\`{file}$fname{apo}\`{rst} failed. No available download tool? " \
          "One of{ehi}:{rst} {cmd}${(pj:$tool_sep:)${=:-curl wget lftp lynx}}{rst}."
          return 1
        fi
      fi
...
.zi-setup-plugin-dir() {
  ...
  if [[ $tpe != tarball ]]; then
    if [[ -z $update ]]; then
      ...
      if (( $+ICE[pack] )); then
        local infix_m="({b}{ice}pack{apo}''{rst}) "
        +zi-message "{nl}Downloading{ehi}:{rst} " \
        "$infix_m{pid}$user${user:+/}$plugin{…}${${${id_as:#$user/$plugin}}:+$id_msg_part}"
      fi
    fi
    local site
    ...
    if [[ $site = */releases ]]; then
      ...
      (
        ...
        for REPLY ( $reply ) {
          ...
          ${${(M)ICE[extract]:#!([^!]|(#e))*}:+--move} ${${(M)ICE[extract]:#!!*}:+--move2}
        }
        return $?
      ) || {
        return 1
      ...
      case ${ICE[proto]} in
        (|https|git|http|ftp|ftps|rsync|ssh)
          :zi-git-clone() {
            ...
            ${(s: :)ICE[cloneopts]---recursive} ${(s: :)ICE[depth]:+--depth {ICE[depth]}} \
    ...
    if [[ $update != -u ]]; then
      ...
      for key in "${reply[@]}"; do
        ...
        if [[ "$hook_rc" -ne 0 ]]; then
          retval="$hook_rc"
          builtin print -Pr -- "${ZI[col-warn]}Warning:%f%b ${ZI[col-obj]}${arr[5]}${ZI[col-warn]}" \
          "hook returned with ${ZI[col-obj]}${hook_rc}${ZI[col-rst]}"
        fi
      done
...
.zi-mirror-using-svn() {
  ...
  if (( ! ${+commands[svn]} )); then
    +zi-message "{error}Warning{ehi}:{rst} Subversion not found{nl}{mmdsh}{rst}" \
    "{auto}Please install it to use \`svn' ice"
  fi
  if [[ "$update" = "-t" ]]; then
...
.zi-forget-completion() {
  ...
  if [[ "${#commands}" -gt 0 ]] && (( quiet == 0 )); then
    +zi-message -n "{auto}Forgetting commands completed by \`$f'{ehi}:{rst} "
  fi
  local k
  integer first=1
  for k ( $commands ) {
...
.zi-compile-plugin() {
  ...
  ${ICE[as]} != command && ${+ICE[binary]} -eq 0 && ( ${+ICE[nocompile]} = 0 || ${ICE[nocompile]} = \! ) ]]; then
    reply=()
    if [[ -n ${ICE[pick]} ]]; then
...
.zi-download-snippet() {
  ...
  command rm -f ${TMPDIR:-/tmp}/zi-execs.$$.lst \
  ${TMPDIR:-/tmp}/zi.installed_comps.$$.lst ${TMPDIR:-/tmp}/zi.skipped_comps.$$.lst ${TMPDIR:-/tmp}/zi.compiled.$$.lst
  ...
  (
    if [[ $url = (http|https|ftp|ftps|scp)://* ]]; then
        ...
        if (( ${+ICE[svn]} )); then
          if [[ $update = -u ]]; then
            ...
            for key in "${reply[@]}"; do
              ...
              "${${key##(zi|z-annex) hook:}%% <->}" update:svn
              hook_rc=$?
              if [[ "$hook_rc" -ne 0 ]]; then
                retval="$hook_rc"
                builtin print -Pr -- "${ZI[col-warn]}Warning:%f%b ${ZI[col-obj]}${arr[5]}${ZI[col-warn]}" \
          ...
          if [[ ${ICE[as]} != command ]]; then
            if [[ -n ${ICE[pick]} ]]; then
              ...
              list=(
                $local_dir/$dirname/*.plugin.zsh(DN)
                $local_dir/$dirname/*.zsh-theme(DN)
                $local_dir/$dirname/init.zsh(DN)
                $local_dir/$dirname/*.zsh(DN)
                $local_dir/$dirname/*.sh(DN)
                $local_dir/$dirname/.zshrc(DN)
              )
            fi
            if [[ -e ${list[1]} && ${list[1]} != */dev/null ]] && \
            [[ -z ${ICE[(i)(\!|)(sh|bash|ksh|csh)]} && ${+ICE[nocompile]} -eq 0 ]]; then
              () {
                builtin emulate -LR zsh -o extended_glob ${=${options[xtrace]:#off}:+-o xtrace}
      ...
      if [[ $ICE[as] != command && ${+ICE[svn]} -eq 0 ]]; then
        ...
        if [[ -e $file_path ]] && \
        [[ -z ${ICE[(i)(\!|)(sh|bash|ksh|csh)]} && $file_path != */dev/null && ${+ICE[nocompile]} -eq 0 ]]; then
          () {
            builtin emulate -LR zsh -o extended_glob ${=${options[xtrace]:#off}:+-o xtrace}
      ...
      if [[ $update = -u ]]; then
        ...
        for key in "${reply[@]}"; do
          arr=( "${(Q)${(z@)ZI_EXTS[$key]:-$ZI_EXTS2[$key]}[@]}" )
          "${arr[5]}" snippet "$save_url" "$id_as" "$local_dir/$dirname" \
          "${${key##(zi|z-annex) hook:}%% <->}" update:file
          hook_rc="$?"
          if [[ "$hook_rc" -ne 0 ]]; then
            retval="$hook_rc"
            builtin print -Pr -- "${ZI[col-warn]}Warning:%f%b ${ZI[col-obj]}${arr[5]}${ZI[col-warn]}" \
      ...
      if [[ ! -e $url ]]; then
        if (( ! OPTS[opt_-q,--quiet] )); then
          zi-message "{error}Error{ehi}:{rst} {msg}The source file {file}$url{msg} doesn't exist.{rst}"
        fi
        retval=4
      fi
      if [[ -e $url && ! -f $url && $url != /dev/null ]]; then
        if (( ! OPTS[opt_-q,--quiet] )); then
          +zi-message "{error}Error{ehi}:{rst} {msg}The source {file}$url{msg} isn't a regular file.{rst}"
        fi
        retval=4
      fi
      if [[ -e $url && ! -r $url && $url != /dev/null ]]; then
        if (( ! OPTS[opt_-q,--quiet] )); then
          +zi-message "{error}Error{ehi}:{rst} {msg}The source {file}$url{msg} isn't" \
    ...
    if (( retval == 4 )); then
      command rmdir "$local_dir/$dirname" 2>/dev/null
      return $retval
    fi
    ...
    if [[ $update = -u ]]; then
      ...
      for key in "${reply[@]}"; do
        arr=( "${(Q)${(z@)ZI_EXTS[$key]:-$ZI_EXTS2[$key]}[@]}" )
        "${arr[5]}" snippet "$save_url" "$id_as" "$local_dir/$dirname" \
        "${${key##(zi|z-annex) hook:}%% <->}" update:$ZI[annex-multi-flag:pull-active]
        hook_rc=$?
        if [[ "$hook_rc" -ne 0 ]]; then
          retval="$hook_rc"
          builtin print -Pr -- "${ZI[col-warn]}Warning:%f%b ${ZI[col-obj]}${arr[5]}${ZI[col-warn]}" \
...
.zi-update-snippet() {
  ...
  if (( ${#tmp} > 1 && ${#tmp} % 2 == 0 )); then
    ...
    +zi-message "{error}Warning{ehi}:{rst}" \
    "{msg2}Inconsistency #3 occurred, please report the string: \`{obj}${ZI_SICE[$id_as]}{msg2}' to the" \
...
∞zi-reset-hook() {
  ...
  ( ${+ICE[reset]} && ZI[-r/--reset-opt-hook-has-been-run] == 1 ) )); then
    ...
    if [[ $type == snippet ]]; then
      if (( $+ICE[svn] )); then
        if [[ $skip_pull -eq 0 && -d $filename/.svn ]]; then
          if (( ! OPTS[opt_-q,--quiet] )); then
            +zi-message "{pre}reset ($msg_bit){ehi}:{rst} {msg}Resetting the repository ($msg_bit) with command{ehi}:" \
            "{rst}svn revert --recursive {…}/{file}$filename/.{rst} {…}"
          ...
          if (( ! OPTS[opt_-q,--quiet] )); then
            if [[ -f $local_dir/$dirname/$filename ]]; then
              if [[ -n $option || -z $ICE[reset] ]]; then
                +zi-message "{pre}reset ($msg_bit){ehi}:{rst}" \
                "{msg}Removing the snippet-file{ehi}:{rst} {file}$filename{msg} {…}{rst}"
              else
                +zi-message "{pre}reset ($msg_bit){ehi}:{rst} " \
                "{msg}Removing the snippet-file{ehi}:{rst} {file}$filename{msg}, with" \
                "the supplied code{ehi}:{rst} {data2}$ICE[reset]{msg} {…}{rst}"
              fi
              if (( option )); then
              ...
              +zi-message "{pre}reset ($msg_bit){ehi}:{rst} {msg}The file " \
              "{file}$filename{msg} is already deleted {…}{rst}"
              if [[ -n $ICE[reset] && ! -n $option ]]; then
                +zi-message "{pre}reset ($msg_bit){ehi}:{rst} " \
                "{msg}(skipped running the provided reset-code{ehi}:{rst} {data2}$ICE[reset]{msg}){rst}"
              fi
            fi
          fi
        else
          if [[ -f $local_dir/$dirname/$filename ]]; then
            +zi-message "{pre}reset ($msg_bit){ehi}:{rst} {msg}Skipping the removal of " \
            "{file}$filename{msg} as there is no new copy scheduled for download{rst}"
          else
            +zi-message "{pre}reset ($msg_bit){ehi}:{rst} {msg}The file " \
            "{file}$filename{msg} is already deleted and no new download is being scheduled{rst}"
          fi
        fi
      fi
      ...
      if (( is_release && !skip_pull )); then
        if (( option )); then
          if (( ! OPTS[opt_-q,--quiet] )); then
            +zi-message "{pre}reset ($msg_bit){ehi}:{rst} {msg}Running{ehi}:{rst} rm -rf " \
            "${${ZI[PLUGINS_DIR]:#[/[:space:]]##}:-${TMPDIR:-/tmp}/xyzabc312}/${${(M)${local_dir##${ZI[PLUGINS_DIR]}[/[:space:]]#}:#[^/]*}:-${TMPDIR:-/tmp}/xyzabc312-zi-protection-triggered}/*"
          ...
          if (( ! OPTS[opt_-q,--quiet] )); then
            +zi-message "{pre}reset ($msg_bit){ehi}:{rst} {msg}Running{ehi}:{rst} ${ICE[reset]:-rm -rf " \
            "${${ZI[PLUGINS_DIR]:#[/[:space:]]##}:-${TMPDIR:-/tmp}/xyzabc312}/${${(M)${local_dir##${ZI[PLUGINS_DIR]}[/[:space:]]#}:#[^/]*}:-${TMPDIR:-/tmp}/xyzabc312-zi-protection-triggered}/*}"
        ...
        if (( option )); then
          +zi-message "{pre}reset ($msg_bit){ehi}:{rst} " \
          "{msg}Resetting the repository with command{ehi}:{rst} {auto}git reset --hard HEAD {…}"
          command git reset --hard HEAD
        else
          +zi-message "{pre}reset ($msg_bit){ehi}:{rst} " \
          "{msg}Resetting the repository with command{ehi}:{rst} {auto}${ICE[reset]:-git reset --hard HEAD} {…}"
...
∞zi-compile-plugin-hook() {
  if [[ "$1" = plugin ]]; then
    ...
    if ! [[ ( $hook = *\!at(clone|pull)* && \
    ${+ICE[nocompile]} -eq 0 ) || ( $hook = at(clone|pull)* && $ICE[nocompile] = '!' ) ]]; then
      return 0
    fi
    # Compile plugin
    if [[ -z $ICE[(i)(\!|)(sh|bash|ksh|csh)] ]]; then
...
∞zi-ps-on-update-hook() {
  ...
  if (( ! OPTS[opt_-q,--quiet] )); then
    +zi-message "Running $tpe's provided update code{ehi}:{rst}" \
    "{info}${ICE[ps-on-update][1,50]}${ICE[ps-on-update][51]:+…}{rst}"
    (
      builtin cd -q "$dir" || return 1
